...
- **当前状态**：用户已明确授权复核修复030–032并完成033–036；实现/本地验证已完成，最终交付SHA与托管状态见[PR2](https://github.com/GabrielMu2006/Palimpsest/pull/2)。早期Proposed文字为历史计划，不覆盖当前授权。
- **RSS补充**：按用户减少重复检验的要求及ADR0028/0029，修正后长跑/RSS一次、各规模冷RSS一次；保留n=1限制，不放宽预算。
...
- **推荐的外部操作范围**：027–033 先本地实施；034 的 hosted CI 验收需要提交并
  ...
  当前已建立该分支的Draft PR2；不merge、不改变main/其他PR或任何OpenCode配置。
...
源码/ADR 为准。运行结果以当前报告和原始证据为准；历史计划本身不能冒充已运行的结果。
